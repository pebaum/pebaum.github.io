// Copied from former midigenerator/script.js (renamed directory)
// Generative Tape Loop MIDI Mobile
// Author: Auto-migrated by Copilot agent
...
class ToneEngine {
  ...
  verb.buffer = this._makeImpulse(2.8, 2.2);
  ...
  // Connect verb & dry to out via gentle EQ
  dry.connect(hp).connect(lp).connect(out);
  verb.connect(wet).connect(hp);
  return { out, verb, wet, dry };
  }
  ...
  playNote(note, velocity=0.6, duration=1, startTime=null){
    const t = (startTime==null)? this.ctx.currentTime : startTime;
  ...
  const stopT = t+Math.max(0.05,duration*1.25);
  fundamental.stop(stopT); third.stop(stopT); fifth.stop(stopT); slight.stop(stopT);
  }
...
class Scheduler {
  ...
  _scheduleBeat(index, time){
    ...
    for(const loop of this.loops){
      if(!loop.enabled || loop.notes.length===0) continue;
      const currPos = ((index * loop.rate) % loop.lengthBeats + loop.lengthBeats) % loop.lengthBeats;
      const prevPos = prevIndex < 0 ? currPos : ((prevIndex * loop.rate) % loop.lengthBeats + loop.lengthBeats) % loop.lengthBeats;
      const wrapped = prevIndex >=0 && prevPos > currPos + 1e-9; // loop wrapped around
      for(const n of loop.notes){
        let inWindow=false;
        if(prevIndex<0){ // first beat: trigger any note at 0
          inWindow = (Math.abs(n.beat) < 1e-6);
        } else if(!wrapped){
