/* FORWARD Simulator (Clean Rewrite) */
// (Full file replaced with consolidated architecture)
// See top-of-file comment in previous patch for feature list.
...
// ---------------- Game Flow ----------------
function startGame(){ RNG.applySeed(seedInput.value.trim()); resetState(); hud.log.textContent=''; summaryPanel.style.display='none'; log('Seed '+RNG.seed); buildDeck(); G.locationOrder=RNG.shuffle([...LOCATIONS_BASE]); buttons.start.disabled=true; buttons.reset.disabled=false; buttons.next.disabled=false; buttons.dragon.disabled=true; nextLocation(); }
...
// ---------------- Initial Load ----------------
resetState(); updateHUD(); log('Enter seed (optional) & Start Game.');
...
log('Note: Mass bury removed; only explicit bury effects now.');
